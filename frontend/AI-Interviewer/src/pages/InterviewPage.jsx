...
import { X, Mic, Square, Bot, User, Volume2, Clock } from "lucide-react";
...
export default function InterviewPage() {
  ...
  const [phase, setPhase] = useState("interview"); // 'interview' | 'feedback' | 'no-score'
  ...
  const [timeLeft, setTimeLeft] = useState(null); // null = not active
  ...
  useEffect(() => {
    if (isSpeaking) {
      setStatus("speaking");
      stopTimer(); // don't count down while AI is speaking
    } else if (!processing) {
      setStatus("idle");
    }
  }, [isSpeaking]);
  ...
  const handleTimeout = useCallback(async () => {
    if (processing || phase !== "interview") return;
    console.log("⏰ Timeout — user didn't respond");

    setMessages((prev) => [
      ...prev,
      { role: "user", content: "⏰ (No response within 30 seconds)", isTimeout: true },
    ]);
    setProcessing(true);
    setStatus("processing");

    try {
      const { data } = await interviewService.respond(
        sessionId,
      ...
      speak(data.message);
      if (data.isComplete) setTimeout(() => handleEndInterview(), 2000);
    } catch (err) {
  ...
  }, [processing, phase, sessionId, speak]);
  ...
  const handleTranscript = useCallback(
    async (text) => {
      ...
      try {
        const { data } = await interviewService.respond(sessionId, text, false);
        setMessages((prev) => [...prev, { role: "assistant", content: data.message }]);
        setQuestionNum(data.questionNumber);
        setMaxQuestions(data.maxQuestions);
        speak(data.message);
    ...
    [sessionId, speak, stop, stopTimer]
  );
  ...
  const { isListening, interimTranscript, startListening, stopListening } =
    useSpeechRecognition({ onTranscript: handleTranscript, onError: handleError });
  ...
  useEffect(() => {
    if (isListening) {
      setStatus("listening");
      stopTimer(); // pause countdown while user is actively speaking
    } else if (!processing && !isSpeaking) {
      setStatus("idle");
    }
  }, [isListening]);

  // Start timer when AI finishes speaking (user's turn to answer)
  useEffect(() => {
    if (!isSpeaking && !processing && phase === "interview" && messages.length > 0) {
      const lastMsg = messages[messages.length - 1];
      if (lastMsg?.role === "assistant") {
        // Short delay so user sees the message first
        const delay = setTimeout(() => startTimer(), 1000);
        return () => clearTimeout(delay);
      }
    }
  }, [isSpeaking, processing, messages]);

  // ── Load first AI message ─────────────────────────────────────────────────
  useEffect(() => {
    if (state?.firstMessage) {
      setMessages([{ role: "assistant", content: state.firstMessage }]);
      setMaxQuestions(state.maxQuestions || 10);
      setTimeout(() => speak(state.firstMessage), 500);
    }
  }, []);
  ...
  const handleEndInterview = async () => {
    ...
    try {
      ...
      if (data.noScore) {
        setFeedbackMeta({ duration: data.durationMinutes, total: data.totalQuestions, message: data.message });
        setPhase("no-score");
      } else {
        setFeedback(data.feedback);
        setFeedbackMeta({ duration: data.durationMinutes, total: data.totalQuestions, answered: data.userResponseCount });
        setPhase("feedback");
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-slate-950">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-2xl font-semibold text-white mb-3">No Score Available</h1>
          <p className="text-slate-400 mb-6 leading-relaxed">{feedbackMeta.message}</p>
          <div className="text-sm text-slate-500 mb-8">
            Duration: {feedbackMeta.duration} min · {feedbackMeta.total} questions asked
          </div>
          <div className="flex gap-3 justify-center">
            <button onClick={() => navigate("/setup")} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-3 rounded-xl transition-all">
              Try Again
            </button>
            <button onClick={() => navigate("/dashboard")} className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-medium px-6 py-3 rounded-xl transition-all">
              Dashboard
            </button>
          </div>
        </div>
      ...
      <div className="min-h-screen px-4 py-8 bg-slate-950">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-slate-500 text-sm mb-6">
            Answered {feedbackMeta.answered} of {feedbackMeta.total} questions · {feedbackMeta.duration} min
          </p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
              ...
              <ul className="space-y-2">{feedback?.strengths?.map((s, i) => <li key={i} className="text-slate-300 text-xs">{s}</li>)}</ul>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
              <h3 className="text-yellow-400 font-medium text-sm mb-3">📈 Improve</h3>
              <ul className="space-y-2">{feedback?.improvements?.map((s, i) => <li key={i} className="text-slate-300 text-xs">{s}</li>)}</ul>
            </div>
          </div>

          <div className="flex gap-3">
            <button onClick={() => navigate("/setup")} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-3 rounded-xl transition-all">Try Again</button>
            <button onClick={() => navigate("/dashboard")} className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-medium px-6 py-3 rounded-xl transition-all">Dashboard</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950" style={{ maxHeight: "100vh" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            ...
            <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
              msg.role === "assistant"
                ...
                : msg.isTimeout
                  ? "bg-slate-800/40 text-slate-500 border border-dashed border-slate-700 rounded-tr-sm italic"
        ...
        {interimText && (
          <div className="flex gap-3 flex-row-reverse">
            ...
            <div className="max-w-[80%] px-4 py-3 rounded-2xl rounded-tr-sm text-sm bg-slate-800/50 text-slate-400 border border-dashed border-slate-600 italic">
              {interimText}...
            </div>
          </div>
        )}
        ...
        {status === "processing" && (
          <div className="flex gap-3">
            ...
            <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-slate-800 flex items-center gap-1">
              {[0,1,2].map((i) => (
      ...
      {/* Bottom controls */}
      <div className="flex-shrink-0 border-t border-slate-800 px-4 pt-4 pb-6">

        {/* 30s countdown timer — shown when idle and timer active */}
        {timeLeft !== null && !isListening && !isSpeaking && !processing && (
          <div className="mb-3 mx-auto max-w-xs">
            <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
              <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Time to respond</span>
              <span className={`font-mono font-bold ${timeLeft <= 10 ? "text-red-400" : "text-slate-300"}`}>
                {timeLeft}s
              </span>
            </div>
            <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${timeLeft <= 10 ? "bg-red-500" : "bg-indigo-500"}`}
                style={{ width: `${(timeLeft / TIMEOUT_SECONDS) * 100}%` }}
              />
            </div>
            <p className="text-xs text-slate-500 text-center mt-1">
              AI will move on if no response in {timeLeft}s
            </p>
          </div>
        )}
          ...
          <div className="relative">
            ...
            >
              {isListening ? <Square className="w-7 h-7 fill-current" />
                : status === "speaking" ? <Volume2 className="w-7 h-7" />
